...
public:
    ...
    void createSSBO(std::vector<uint8_t>& inputData) {
        ...
        /*   VkBufferCreateInfo bufferInfo{};
           ...
           bufferInfo.size = inputData.size() * sizeof(uint8_t);
           bufferInfo.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
           bufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;

           VK_CHECK_RESULT(vkCreateBuffer(device, &bufferInfo, nullptr, &ssbo));

           VkMemoryRequirements memRequirements;
           vkGetBufferMemoryRequirements(device, ssbo, &memRequirements);
           ...
           VK_CHECK_RESULT(vkAllocateMemory(device, &allocInfo, nullptr, &ssboMemory));
           vkBindBufferMemory(device, ssbo, ssboMemory, 0);
           */
    ...
    void recordCommandBuffer(uint32_t numElements) {
        ...
        uint32_t groupCountX = (SAT_WIDTH + 15) / 16;
        uint32_t groupCountY = (SAT_HEIGHT + 15) / 16;
    ...
    void initVulkan(std::vector<uint8_t>& inputData) {
        createInstance();
        pickPhysicalDevice();
